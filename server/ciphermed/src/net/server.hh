...
public:
    ...
    /*add by andy 2017*/
    void run_exes_change_encryption_scheme_helper();    
    /*add end*/

    //add by andy 2018
    void run_Encrypted_LSB_helper();
    void run_SVR_helper();
    void run_SBD_helper();
    void run_SM_helper();
    void run_SBOR_helper();

    void fhe_calc(long op1,long op2);
    void run_GETMINn_helper(int num,size_t bit_size_,COMPARISON_PROTOCOL comparison_prot);
    void run_calcDis_hepler(int train_num,vector<vector<Ctxt>> theStack);
    void run_calcDis_hepler(int train_num,vector<vector<long>> theStack);//for test
   //add end
