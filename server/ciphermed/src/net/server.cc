...
#include <net/defs.hh>  //net/defs.hh
...
void Server::init_FHE_context()
{
    ...
    //create_FHEContext is in util/fhe_util.hh
...
//add by andy 2017
void Server_session::run_exes_change_encryption_scheme_helper() 
{
    EncryptedArray ea(server_->fhe_context(), server_->fhe_G());
    auto pk = server_->paillier_pk();
    Paillier p(pk,rand_state_);
    const FHEPubKey& publicKey = server_->fhe_sk();
    exes_change_encryption_scheme_slots_helper(socket_,p,publicKey,server_->fhe_sk(),ea,server_->paillier());
}
//add end

//add by andy 2018
void Server_session::run_Encrypted_LSB_helper()
{
    exes_Encrypted_LSB_helper(socket_,server_->paillier());
}

void Server_session::run_SVR_helper()
{
     exes_SVR_helper(socket_,server_->paillier());
}

void Server_session::run_SBD_helper()
{
    exes_SBD_helper(socket_,server_->paillier());
}


void Server_session::run_SM_helper()
{
   exes_SM_helper(socket_,server_->paillier(),N);
}

void Server_session::run_SBOR_helper()
{
    exes_SBOR_helper(socket_,server_->paillier(),N);
}

void Server_session::fhe_calc(long op1,long op2)
{
    const FHEPubKey& publicKey = server_->fhe_sk();
    EncryptedArray ea(server_->fhe_context(), server_->fhe_G());
    exes_FHE_Calc(socket_,op1,op2,publicKey,server_->fhe_sk(),ea);
}

void Server_session::run_GETMINn_helper(int num,size_t bit_size_,COMPARISON_PROTOCOL GC_PROTOCOL)
{
	...
	{
            help_enc_comparison(bit_size_, GC_PROTOCOL);
        }
        num = ceil(num / 2.0);
    }
    
    /*exes_GETMINn_Right_helper(socket_,server_->paillier());*/
}

//calc the distance between test and train
void Server_session::run_calcDis_hepler(int train_num,vector<vector<Ctxt>> theStack)
{
     const FHEPubKey& publicKey = server_->fhe_sk();
     EncryptedArray ea(server_->fhe_context(), server_->fhe_G());
     exes_calcDis_helper(socket_,train_num,theStack,publicKey,server_->fhe_sk(),ea);
}

// for test
void Server_session::run_calcDis_hepler(int train_num,vector<vector<long>> theStack)
{
    const FHEPubKey& publicKey = server_->fhe_sk();
    EncryptedArray ea(server_->fhe_context(), server_->fhe_G());
    ...
    NewPlaintextArray p(ea);
     ...
     cout<<"encrypted the data"<<endl;
     vector<Ctxt> enc_v1;
     vector<vector<Ctxt>> enc_v2;

     for(int i=0;i<train_num;i++)
     {
         vector<long> vec = theStack[i];
         ...
         enc_v2.push_back(enc_v1);
         enc_v1.clear();
     }
      
     exes_calcDis_helper(socket_,train_num,enc_v2,publicKey,server_->fhe_sk(),ea);
}
