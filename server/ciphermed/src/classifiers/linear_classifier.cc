...
void Bench_Linear_Classifier_Server_session::run_session()
{
    try {
        ...
        double compare_time = 0., dot_prod_time = 0.;
        Timer t;
...
//            cout << "Round #" << i << " done" << endl;
        ...
        cout << "Compare time: " << compare_time/nRounds << endl;
        cout << "Dot product time: " << dot_prod_time/nRounds << endl;
...
/*
    ...
    for(unsigned int i=0; i<5; i++){
       ...
       help_enc_comparison(nbits, GC_PROTOCOL);
       delete t;       
       cout<<"nbits : "<<nbits<<endl;
       compare_time+=GET_BENCHMARK_TIME;
       
       Stime[i] = compare_time;
       nbits = 2*nbits;
#ifdef BENCHMARK
...
void Bench_Linear_Classifier_Server_session::writeToFile(double a[],size_t size){
   ofstream outfile("server_64_t_1024.txt", std::ios::app);
   if (!outfile)
   {
