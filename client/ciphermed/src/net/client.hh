...
public:
    ...
    //add by andy 2017
    vector<mpz_class> change_encryption_scheme(const vector<Ctxt> &c_fhe);
    //add end

    //add by andy 2018
    mpz_class Encrypted_LSB(mpz_class T,mpz_class i);
    mpz_class SVR(mpz_class E_x,vector<mpz_class>vec_Ex,int m);
    vector<mpz_class> SBD(mpz_class E_x,int m);
    mpz_class SM(mpz_class e_a,mpz_class e_b);
    mpz_class SBOR(mpz_class o1,mpz_class o2);
    void run_FHE_add_helper();
    KNNNode getMINn(vector<KNNNode> d,size_t bit_size_,COMPARISON_PROTOCOL comparison_prot);
    vector<Ctxt> calcDis(vector<long> test_data,mpz_class n1,mpz_class n2);
