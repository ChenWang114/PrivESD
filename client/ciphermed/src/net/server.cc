...
void Server_session::send_gm_pk()
{
    ...
    Protobuf::GM_PK pk_message = get_pk_message(&(server_->gm()));// protobuf/protobuf_conversion
...
//add by andy 2017
void Server_session::run_exec_change_encryption_scheme_helper()
{
    EncryptedArray ea(server_->fhe_context(), server_->fhe_G());
    auto pk = server_->paillier_pk();
    Paillier p(pk,rand_state_);
    exec_change_encryption_scheme_slots_helper(socket_,p,server_->fhe_sk(),ea);
}
//add end
