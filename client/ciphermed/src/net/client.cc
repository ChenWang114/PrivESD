...
void Client::get_server_pk_paillier()
{
    ...
    N = create_from_pk_message(pk);//get mod N
}

//add by andy :get mod N
mpz_class Client::get_paillier_N()
{
    return N;
}
...
//add by andy 2017
vector<mpz_class> Client::change_encryption_scheme(const vector<Ctxt> &c_fhe)
{
    cout<<"client change_encryption_scheme testing"<<endl;
    EncryptedArray ea(*fhe_context_, fhe_G_);
    return exec_change_encryption_scheme_slots(socket_,c_fhe,*server_paillier_,*server_fhe_pk_,ea,rand_state_);
}
//add end 2017
...
mpz_class Client::Encrypted_LSB(mpz_class T,mpz_class i)
{	
    cout<<"Encrypted_LSB client start"<<endl;
    return exec_Encrypted_LSB(socket_,T,i,N,rand_state_,*server_paillier_);//net/exec_protocol.cc
}

//验证SBD位分解是否正确
mpz_class Client::SVR(mpz_class E_x,vector<mpz_class>vec_Ex,int m)
{
    cout<<"SVR starting............."<<endl;
    return exec_SVR(socket_,E_x,vec_Ex,m,N,rand_state_,*server_paillier_);
}

vector<mpz_class> Client::SBD(mpz_class E_x,int m)
{
    cout<<"SBD starting..........."<<endl;
    return exec_SBD(socket_,E_x,N,m,rand_state_,*server_paillier_);
}

mpz_class Client::SM(mpz_class e_a,mpz_class e_b)
{
    cout<<"SM testing........."<<endl;
    return exec_SM(socket_,e_a,e_b,N,*server_paillier_,rand_state_);
}

mpz_class Client::SBOR(mpz_class o1,mpz_class o2)
{
    cout<<"SBOR testing........"<<endl;
    return exec_SBOR(socket_,o1,o2,N,*server_paillier_,rand_state_);
}

void Client::run_FHE_add_helper()
{
    exec_FHE_add_helper(socket_,*server_fhe_pk_);
}

KNNNode Client::getMINn(vector<KNNNode> d,size_t bit_size_,COMPARISON_PROTOCOL GC_PROTOCOL)
{
   vector<KNNNode> d_ ;
   KNNNode node(-1, -1, -1);
   d_.push_back(node);
   ...
   int num = d_.size() - 1;
   int length = ceil(log10(num) / log10(2));
   cout<<"length : "<<length<<endl;
   KNNNode *node2;
   for (int i = 1; i <= length; i++)
   {
	int jlen = floor(num / 2.0);
	for (int j = 1; j <= jlen; j++)
	{
	     if (i == 1)
	     {
		if (!enc_comparison(d_[2 * j - 1].getDistance(), d_[2 * j].getDistance(),bit_size_,GC_PROTOCOL))//比较协议
		{	
		   d_[2 * j - 1].setDistance(d_[2 * j].getDistance());//下标和类别也要做记录
		   d_[2 * j - 1].setIndex(d_[2 * j].getIndex());
		   d_[2 * j - 1].setC(d_[2 * j].getC());
		   d_[2 * j].setDistance(0);
		}
	        else{
		   d_[2 * j].setDistance(0);	
	        }
		node2 = new KNNNode(d_[2 * j - 1].getIndex(), d_[2 * j - 1].getDistance(),d_[2 * j - 1].getC());
	     }
	     else
	     {
		if (!enc_comparison(d_[1 + (j - 1)*pow(2, i)].getDistance(), d_[1 + (j - 1)*pow(2, i) + pow(2, i - 1)].getDistance(),bit_size_,GC_PROTOCOL))
		{
		   d_[1 + (j - 1)*pow(2, i)].setDistance(d_[1 + (j - 1)*pow(2, i) + pow(2, i - 1)].getDistance());//下标和类别也要做记录
		   d_[1 + (j - 1)*pow(2, i)].setIndex(d_[1 + (j - 1)*pow(2, i) + pow(2, i - 1)].getIndex());
		   d_[1 + (j - 1)*pow(2, i)].setC(d_[1 + (j - 1)*pow(2, i) + pow(2, i - 1)].getC());
		   d_[1 + (j - 1)*pow(2, i) + pow(2, i - 1)].setDistance(0);
		}			
		else
		{
		   d_[1 + (j - 1)*pow(2, i) + pow(2, i - 1)].setDistance(0);
		}
		node2 = new KNNNode(d_[1 + (j - 1)*pow(2, i)].getIndex(),d_[1 + (j - 1)*pow(2, i)].getDistance(), d_[1 + (j - 1)*pow(2, i)].getC());
	     }
	}
	num = ceil(num / 2.0);
   }
   ...
   // test result 
   /*mpz_class dis = node2->getDistance();
   exec_GETMINn_Right(socket_,dis);  
*/
   return *node2;
...
vector<Ctxt> Client::calcDis(vector<long> test_data,mpz_class n1,mpz_class n2)
{
     Ctxt c(*server_fhe_pk_);
     EncryptedArray ea(*fhe_context_, fhe_G_); 
     NewPlaintextArray p(ea); 
     vector<Ctxt> test_fhe;
     for(int i=0;i<test_data.size();i++)
     {
         encode(ea,p,test_data[i]);
	 ea.encrypt(c, *server_fhe_pk_, p);
	 test_fhe.push_back(c);
     }
     return exec_calcDis(socket_,test_fhe,n1,n2,*server_fhe_pk_);
}
